...
for (i = 0; i < listSource.length; i++) {
	...
	*/
	...
	//dirSave = getDir("pick save destination");
	Table.save(dirSave + title + "BranchInfo.csv");
	run("Close");
	selectWindow("Results");
	Table.save(dirSave + title + "Results.csv");
	run("Close");
	selectWindow(title + "_skeleton");
	...
	while(moreCells){
		...
		//dirSave = getDir("pick save destination");
		Table.save(dirSave + title +"HullCircle_"+ cellNum +".csv");
		run("Close");
		selectWindow("Box Count Summary FileFracLac 2015Sep090313a9330");
		Table.save(dirSave + title + "BoxCountSummary_"+ cellNum +".csv");
		run("Close");
		roiManager("save", dirSave + title + "cell_" + cellNum + "_ROI.zip");
		roiManager("reset");
